...
import sys
sys.path.append(os.path.abspath(os.path.join(os.getcwd(), "modules/structure_module")))
...
from configs.configs import logger, SA_HOME, DEBUG, PRED_DIR, VIEWER_DATA_DIR, NUM_WORKERS
...
def switchPred(algo):
    if algo == "highlighter":
        ...
        return MsafAlgosBdryOnly("olda", USE_MODEL_DIC["olda"])
    elif algo == "mixed":
...
@click.option(
    ...
    nargs=1,
    type=click.Choice(
        ["seqRecur", "seqRecurS", "multi", "single", "highlighter", "olda", "mixed"]
    ),
    default="single",
)
@click.option(
...
def main(audiofiles, outputdir, metaoutputdir, algo, force, workers):
    ...
    for i, pair in enumerate(ddataset.pathPairs):
        ...
        figurePath = os.path.join(os.getcwd(), f"{SA_HOME}/data/test/predict_{audioFileName}.svg")
        plt.savefig(figurePath, bbox_inches="tight")
        writeJsonMetadata(audiofile, mergeIntervals(mirexFmt), figurePath, metaOutput)
        if DEBUG:
            plt.show()

def get_sections(audiofile, algo="single", force=True, workers=NUM_WORKERS):
    logger.debug(f"algo={algo}")
    logger.info(f"preprocess to generate features")
